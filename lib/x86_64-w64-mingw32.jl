...
"""
struct __JL_Ctag_492
    data::NTuple{8, UInt8}
end

function Base.getproperty(x::Ptr{__JL_Ctag_492}, f::Symbol)
    f === :scheduled && return Ptr{Bool}(x + 0)
    f === :reserved && return Ptr{Csize_t}(x + 0)
    return getfield(x, f)
end

function Base.getproperty(x::__JL_Ctag_492, f::Symbol)
    r = Ref{__JL_Ctag_492}(x)
    ptr = Base.unsafe_convert(Ptr{__JL_Ctag_492}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{__JL_Ctag_492}, f::Symbol, v)
    unsafe_store!(getproperty(x, f), v)
end
...
function Base.getproperty(x::Ptr{aws_task}, f::Symbol)
    ...
    f === :abi_extension && return Ptr{__JL_Ctag_492}(x + 56)
    return getfield(x, f)
end
