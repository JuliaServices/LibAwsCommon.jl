...
"""
struct __JL_Ctag_499
    data::NTuple{8, UInt8}
end

function Base.getproperty(x::Ptr{__JL_Ctag_499}, f::Symbol)
    f === :scheduled && return Ptr{Bool}(x + 0)
    f === :reserved && return Ptr{Csize_t}(x + 0)
    return getfield(x, f)
end

function Base.getproperty(x::__JL_Ctag_499, f::Symbol)
    r = Ref{__JL_Ctag_499}(x)
    ptr = Base.unsafe_convert(Ptr{__JL_Ctag_499}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{__JL_Ctag_499}, f::Symbol, v)
    unsafe_store!(getproperty(x, f), v)
end
...
function Base.getproperty(x::Ptr{aws_task}, f::Symbol)
    ...
    f === :abi_extension && return Ptr{__JL_Ctag_499}(x + 56)
    return getfield(x, f)
end
...
"""
@cenum __JL_Ctag_351::UInt32 begin
    AWS_UUID_STR_LEN = 37
end
