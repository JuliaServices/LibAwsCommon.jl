...
function Base.propertynames(x::tm, private::Bool = false)
    (:tm_sec, :tm_min, :tm_hour, :tm_mday, :tm_mon, :tm_year, :tm_wday, :tm_yday, :tm_isdst, if private
            fieldnames(typeof(x))
        else
            ()
        end...)
...
function Base.propertynames(x::aws_date_time, private::Bool = false)
    (:timestamp, :milliseconds, :tz, :gmt_time, :local_time, :utc_assumed, if private
            fieldnames(typeof(x))
        else
            ()
        end...)
...
function Base.getproperty(x::Ptr{aws_task}, f::Symbol)
    ...
    f === :abi_extension && return Ptr{__JL_Ctag_10}(x + 56)
    return getfield(x, f)
end
...
function Base.propertynames(x::aws_thread_once, private::Bool = false)
    (:ptr, if private
            fieldnames(typeof(x))
        else
            ()
